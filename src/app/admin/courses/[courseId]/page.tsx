...
import { ArrowLeft, Save, Settings, List, Plus, Trash2, GripVertical, Video, Link2 } from "lucide-react";

// Mock Data structure for the editor state
...
export default function CourseEditorPage({ params }: { params: { courseId: string } }) {
    ...
    // Local state for the entire course to allow "editing everything"
    ...
    const handleUpdateWeek = (weekId: string, newTitle: string) => {
        setCourse({
            ...course,
            weeks: course.weeks.map(w => w.id === weekId ? { ...w, title: newTitle } : w)
        });
    };
        ...
        setCourse({
            ...
            weeks: course.weeks.map(w => {
                if (w.id === weekId) {
                    return {
                        ...w,
                        days: w.days.map(d => d.id === dayId ? { ...d, [field]: value } : d)
                    };
                }
                return w;
            })
    ...
    const handleSave = () => {
        alert("Cambios guardados localmente (Mock). En el siguiente sprint se conectará a Prisma.");
    ...
    return (
        <div className="flex flex-col gap-6 h-[calc(100vh-80px)]">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[var(--color-glass-border)] pb-6">
                ...
                <button
                    onClick={handleSave}
                    className="bg-[var(--color-primary)] hover:bg-blue-600 text-white font-semibold py-2.5 px-6 rounded-xl transition-all duration-300 glow-accent flex items-center gap-2"
                >
                    <Save size={18} />
                    Guardar Cambios
                </button>
            ...
            <div className="flex flex-col lg:flex-row gap-8 flex-1 overflow-hidden">
                ...
                {/* Right Content Area */}
                <div className="flex-1 glass-effect rounded-2xl border border-[var(--color-glass-border)] overflow-y-auto custom-scrollbar p-6">
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm text-slate-400">Construye la estructura de tu curso. Los cambios se guardan localmente hasta conectar la base de datos.</p>
                                </div>
                                <button
