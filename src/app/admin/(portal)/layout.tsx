        ...
        <div className="flex h-screen bg-[var(--background)] overflow-hidden">
            <aside className="w-64 flex-shrink-0 flex flex-col border-r border-[var(--color-glass-border)] bg-[var(--color-background-dark)]/90 backdrop-blur-xl z-20">
                ...
                <nav className="flex-1 overflow-y-auto p-4 space-y-2">
                    <Link href="/admin" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-300 rounded-xl hover:bg-white/5 hover:text-white transition-colors">
                        <LayoutDashboard size={18} />
                        Dashboard
                    </Link>
                    <Link href="/admin/courses" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-300 rounded-xl hover:bg-white/5 hover:text-white transition-colors">
                        <BookOpen size={18} />
                        Mis Cursos
                    </Link>
                    <Link href="/admin/students" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-300 rounded-xl hover:bg-white/5 hover:text-white transition-colors">
                        <LayoutDashboard size={18} />
                        Estudiantes
                    </Link>
                    ...
                    <Link href="/admin/qa" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-300 rounded-xl hover:bg-white/5 hover:text-white transition-colors relative">
                        ...
                        Q&A Inbox
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-[var(--color-primary)] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">4</span>
                    </Link>
                </nav>
            ...
            <main className="flex-1 relative overflow-y-auto custom-scrollbar">
                ...
                <div className="relative z-10 p-6 sm:p-10 max-w-7xl mx-auto">{children}</div>
            </main>
        </div>
    );
}
