...
const INBOX_QUESTIONS = [
    {
        ...
        day: "Día 1: Introducción a Server Components",
    ...
    },
    {
        ...
        day: "Día 4: Normalización",
        content: "No entiendo la diferencia entre Primera Forma Normal y Segunda Forma Normal. ¿Me pueden dar un ejemplo con una tabla de usuarios?",
        time: "Hace 5 horas",
        status: "pending"
    },
    {
        ...
        day: "Día 2: Herencia y Polimorfismo",
        content: "Si tengo una clase abstracta, ¿puedo instanciarla directamente o siempre necesito una clase hija?",
        time: "Ayer",
        status: "resolved"
    },
];

export default function AdminQAInboxPage() {
    const [activeTab, setActiveTab] = useState("pending");
    const [replyText, setReplyText] = useState("");
    const [questions, setQuestions] = useState(INBOX_QUESTIONS);

    const filteredQuestions = questions.filter(q => q.status === activeTab);

    const handleReply = (id: string, e: React.FormEvent) => {
        e.preventDefault();
        if (!replyText.trim()) return;

        // Simulate resolving the question
        setQuestions(questions.map(q =>
            q.id === id ? { ...q, status: "resolved" } : q
        ));
        setReplyText("");
        alert("Respuesta enviada a " + questions.find(q => q.id === id)?.studentName);
    };
                                ...
                                <div className="p-5 bg-[var(--color-background-dark)] border-t border-[var(--color-glass-border)]">
                                    <form onSubmit={(e) => handleReply(q.id, e)} className="flex flex-col gap-3">
                                        ...
                                        <div className="flex justify-end gap-3">
                                            <button
                                                type="submit"
                                                className="bg-[var(--color-primary)] hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded-lg transition-all duration-300 glow-accent text-sm flex items-center gap-2"
                                            >
